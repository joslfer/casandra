import { t as supabase } from "./client-DyR3hPwK.js";
import { useCallback, useEffect, useMemo, useState } from "react";
//#region src/hooks/useSesion.ts
...
//#endregion
//#region src/hooks/useMercado.ts
/**
...
function useMercado(usuario) {
	...
	const pausado = useMemo(() => alumnos.some((a) => a.id === usuario?.id && a.pausado), [alumnos, usuario]);
	/** leerAsignaturas() */
	const leerAsignaturas = useCallback(() => asignaturas, [asignaturas]);
	/** leerApuestas(): actividad reciente */
	const leerApuestas = useCallback(() => [...apuestas].sort((a, b) => b.cuando - a.cuando).slice(0, 5), [apuestas]);
	/** leerAlumnos() */
	...
	*/
	const leerPreguntas = useCallback((opts) => {
	...
	const apostar = useCallback((id, lado, tokens = 1) => {
	...
	const crearPregunta = useCallback((titulo, asignaturaId) => {
		const t = titulo.trim();
	...
	const resolver = useCallback((id, entro) => {
	...
	const desresolver = useCallback((id) => {
		if (!esAdmin) return false;
	...
	const archivar = useCallback((id, valor = true) => {
		if (!esAdmin) return false;
		setPreguntas((prev) => prev.map((p) => p.id === id ? {
			...p,
			archivada: valor
	...
	const eliminarPregunta = useCallback((id) => {
		if (!esAdmin) return false;
		setPreguntas((prev) => prev.filter((p) => p.id !== id));
		return true;
	}, [esAdmin]);
	const moverPregunta = useCallback((id, asignaturaId) => {
		if (!esAdmin) return false;
		setPreguntas((prev) => prev.map((p) => p.id === id ? {
			...p,
			asignaturaId
	...
	const editarTitulo = useCallback((id, titulo) => {
	...
	const crearAsignatura = useCallback((nombre) => {
	...
	const darTokens = useCallback((alumnoId, delta) => {
		if (!esAdmin) return false;
	...
	const pausarAlumno = useCallback((alumnoId, valor) => {
		if (!esAdmin) return false;
		setAlumnos((prev) => prev.map((a) => a.id === alumnoId ? {
			...a,
			pausado: valor
	...
	const usarHash = useCallback((valor) => {
		setPerfil((p) => ({
			...p,
			usaHash: valor,
...
//#endregion
export { volumen as a, useMercado as i, nombreVisible as n, useSesion as o, probabilidad as r, haceTexto as t };
