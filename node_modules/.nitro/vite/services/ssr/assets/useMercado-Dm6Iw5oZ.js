...
import { s as supabase } from "./client-DKc3rSMY.js";
...
function useMercado(usuario) {
  ...
  const leerAsignaturas = useCallback(() => asignaturas, [asignaturas]);
  const leerAlumnos = useCallback(() => alumnos, [alumnos]);
  ...
  const apostar = useCallback(async (id, lado, tokens = 1) => {
  ...
  const retirar = useCallback(async (id) => {
    if (pausado) return 0;
  ...
  const crearPregunta = useCallback(async (titulo, asignaturaId) => {
    const t = titulo.trim();
  ...
  const resolver = useCallback(async (id, entro) => {
  ...
  const desresolver = useCallback(async (id) => {
    if (!esAdmin) return false;
    await supabase.from("preguntas").update({ resultado: null, archivada: false }).eq("id", id);
    await cargarDatos();
    return true;
  }, [esAdmin, cargarDatos]);
  const archivar = useCallback(async (id, valor = true) => {
    if (!esAdmin) return false;
  ...
  const pausarAlumno = useCallback(async (alumnoId, valor) => {
    if (!esAdmin) return false;
