...
import { s as supabase } from "./client-DKc3rSMY.mjs";
...
function useSesion() {
  ...
  reactExports.useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((_e, session) => {
      setUsuario(
        session?.user ? aUsuario(session.user.id, session.user.email, session.user.user_metadata?.["full_name"]) : null
      );
      setCargando(false);
    });
    supabase.auth.getSession().then(({ data }) => {
      setUsuario(
        data.session?.user ? aUsuario(
...
function useMercado(usuario) {
  const [preguntas, setPreguntas] = reactExports.useState([]);
  const [asignaturas, setAsignaturas] = reactExports.useState([]);
  const [alumnos, setAlumnos] = reactExports.useState([]);
  const [apuestas, setApuestas] = reactExports.useState([]);
  const [miPerfil, setMiPerfil] = reactExports.useState(null);
  const cargarDatos = reactExports.useCallback(async () => {
    ...
    if (resAsig.data) setAsignaturas(resAsig.data.map((a) => ({ id: a.id, nombre: a.nombre })));
    if (resPerf.data) {
  ...
  const perfil = miPerfil ? { nombre: miPerfil.nombre, usaHash: miPerfil.usaHash, hash: miPerfil.hash } : { nombre: "", usaHash: false, hash: "" };
  const miNombre = perfil.usaHash ? `#${perfil.hash}` : perfil.nombre || usuario?.nombre || "Tú";
  const leerAsignaturas = reactExports.useCallback(() => asignaturas, [asignaturas]);
  const leerAlumnos = reactExports.useCallback(() => alumnos, [alumnos]);
  const leerApuestas = reactExports.useCallback(() => {
    const agrupadas = [];
  ...
  const resumen = reactExports.useCallback(() => {
    let ganar = 0;
    let perder = 0;
    const nombres = [];
  ...
  const apostar = reactExports.useCallback(async (id, lado, tokens = 1) => {
  ...
  const retirar = reactExports.useCallback(async (id) => {
    if (pausado) return 0;
    const p = preguntas.find((p2) => p2.id === id);
    const devolucion = p ? p.misSi + p.misNo : 0;
    setPreguntas((prev) => prev.map((p2) => {
      if (p2.id !== id) return p2;
      return {
        ...p2,
        poolSi: Math.max(0, p2.poolSi - p2.misSi),
        poolNo: Math.max(0, p2.poolNo - p2.misNo),
        misSi: 0,
        misNo: 0
      };
    }));
  ...
  const crearPregunta = reactExports.useCallback(async (titulo, asignaturaId) => {
    const t = titulo.trim();
  ...
  const resolver = reactExports.useCallback(async (id, entro) => {
  ...
  const pausarAlumno = reactExports.useCallback(async (alumnoId, valor) => {
    if (!esAdmin) return false;
